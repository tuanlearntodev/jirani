...
- Protect with `Depends(RoleChecker([RoleEnum.admin, RoleEnum.teacher]))`
- Parse request body as `BulkCreateRequest` (see `account_schema.py`)
- **Role restriction**: if `current_user.role == RoleEnum.teacher` and `bulk_data.role != RoleEnum.student`, raise `HTTPException(403, "Teachers can only create student accounts")`
- **No admin creation**: if `bulk_data.role == RoleEnum.admin`, raise `HTTPException(400, "Cannot create admin accounts via bulk endpoint")`
- Call `AuthService.bulk_create_users(bulk_data)` — returns list of accounts + raw passwords
- Return `BulkCreateResponse` with `created` count and list of `{username, password, role}`
- **Username format**: `{prefix}{number:03d}` (e.g., `student001`, `student002`). Determine starting number by querying existing accounts matching the prefix pattern
- **Count limit**: 1-100 accounts per request
...
- Call `AuthService.validate_credentials(current_user.role, change_data.new_password, context="self_change")` to enforce role-appropriate strength rules (student: min 4 chars, teacher/admin: 8+)
...
5. Call `POST /auth/users/bulk` with `count=5, role=student, prefix="classA"` → creates `classA001` through `classA005`
6. Call `GET /auth/me` with the student token → should return student profile
7. Call `GET /auth/users` as teacher → should only see students
8. Login as the student with the auto-generated password → should work
9. As the student, call `POST /auth/change-password` with a new 4+ char password → should work
10. As the teacher, call `POST /auth/reset-password` for the student → should return a new 6-digit password

- [ ] Add `BulkCreateRequest` and `BulkCreateResponse` schemas to `account_schema.py`
- [ ] Add `bulk_create_users()` method to `AuthService`
- [ ] Add `get_next_prefix_number()` method to `AuthRepo`
- [ ] Rewrite `app/api/auth_router.py` with all endpoints including bulk
