...
def get_audio(db: Session = Depends(get_db)):
    tracks = (
        db.query(Audio)
        .options(joinedload(Audio.tags))
        .filter(Audio.deleted_at == None)
        .all()
    )
...
async def upload_audio(
    file: UploadFile = File(...), tags: str = Form(""), db: Session = Depends(get_db)
):
    ...
    tag_names = (
        [t.strip() for t in tags.split(",") if t.strip()] if tags.strip() else []
    )

    audio_db = repo.create_audio(
        Audio_Create(
            title=title,
            description=None,
...
async def upload_multiple(
    ...
    for file in files:
        ...
        title = file.filename.rsplit(".", 1)[0]
        audio_db = repo.create_audio(
            Audio_Create(title=title, description=None, file_path=file_location)
        )
        db.refresh(audio_db)
