...
class BookService:
    ...
    def search_books(
        ...
        books = self.book_repo.search_books(
            title=title, tags=tags, file_type=file_type, extension=extension
        )
    ...
    async def upload_book(
        self, metadata: BookUpload, file: UploadFile, cover: UploadFile | None = None
    ) -> BookBase:
        ...
        if not metadata.title or not metadata.title.strip():
            ...
            metadata.title = (
                raw_name.replace("_", " ").replace("-", " ").title().strip()
            )
        ...
        if "." not in file.filename:
            raise HTTPException(
                status_code=400, detail="Book file must have an extension"
            )
        ...
        try:
            ...
            if cover and cover.filename:
                ...
                if "." not in cover.filename:
                    raise HTTPException(
                        status_code=400, detail="Cover file must have an extension"
                    )
            ...
            if saved_cover_path and saved_cover_path.exists():
                saved_cover_path.unlink()
            raise HTTPException(status_code=500, detail=f"Failed to upload book: {e!s}")

    async def update_book(
        self, book_uid: str, metadata: BookUpload, cover: UploadFile | None = None
    ) -> BookBase:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to update book: {e!s}")
    ...
    def delete_book(self, book_uid: str) -> None:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to delete book: {e!s}")
    ...
    def _validate_book_content(self, content: bytes, extension: str):
        ...
        # Check EPUB signature (it's a ZIP file)
            ...
            if not content.startswith(b"PK\x03\x04"):
                raise HTTPException(
                    status_code=400, detail="File does not appear to be a valid EPUB"
                )
    ...
    def _generate_thumbnail(
        self, book_path: Path, output_path: Path, extension: str
    ) -> bool:
                ...
                with zipfile.ZipFile(str(book_path), "r") as z:
                    ...
                    if opf_path and opf_path in names:
                        ...
                        for item in opf.iter():
                            if strip_ns(item.tag) == "item":
                                if "cover-image" in item.get("properties", ""):
                                    ...
                                    print(
                                        f"DEBUG: found via properties=cover-image: {cover_href}"
                                    )
                                    break
                                if cover_id and item.get("id") == cover_id:
                                    cover_href = item.get("href")
                                    print(
                                        f"DEBUG: found via cover_id match: {cover_href}"
                                    )
                                    break
                        ...
                        if cover_href:
                            ...
                            print(
                                f"DEBUG: full_cover_path: {full_cover_path}, in names: {full_cover_path in names}"
                            )

                            if full_cover_path in names:
                                content = z.read(full_cover_path)
                                if full_cover_path.endswith(
                                    (".xhtml", ".html", ".htm")
                                ):
                                    match = _re.search(
                                        rb'<img[^>]+src=["\']([^"\']+)["\']',
                                    ...
                                    if match:
                                        ...
                                        if img_path in names:
                                            ...
                                            print(
                                                f"DEBUG: found cover via xhtml img: {img_path}"
                                            )
                                else:
                                    cover_bytes = content
                                    print(
                                        f"DEBUG: found cover via manifest direct image: {full_cover_path}"
                                    )
                                            ...
                                            if full_path.endswith(
                                                ...
                                                if match:
                                                    ...
                                                    if img_path in names:
                                                        cover_bytes = z.read(img_path)
                                                        print(
                                                            f"DEBUG: found cover via guide xhtml: {img_path}"
                                                        )
                                        break
